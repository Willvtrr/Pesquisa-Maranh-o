    ...
    <AppLayout>
      ...
      <Dialog open={detailModal.open} onOpenChange={(open) => setDetailModal(prev => ({ ...prev, open }))}>
        <DialogContent className="max-w-[98vw] w-[1300px] h-[92vh] flex flex-col p-0 overflow-hidden bg-white rounded-[2rem] border-none shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)]">
          ...
          <div className="px-8 py-5 border-b border-zinc-50 flex items-center justify-between bg-white relative z-50">
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                <DialogTitle className="text-2xl font-black tracking-tighter text-zinc-950 uppercase leading-none">
            ...
            <div className="pr-12">
              <Badge variant="outline" className="border-zinc-100 text-zinc-400 font-black px-3 py-1 text-[9px] uppercase tracking-widest bg-zinc-50/50">Sincronização em tempo real</Badge>
            </div>
          </div>

          <div className="flex-1 flex overflow-hidden">
            
            <div className="w-[280px] bg-zinc-50/50 border-r border-zinc-50 flex flex-col overflow-hidden shrink-0">
              <div className="p-5 space-y-4">
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-zinc-100 rounded-2xl pl-11 pr-4 py-3 text-[11px] font-black uppercase outline-none shadow-sm focus:border-orange-600 transition-all placeholder:text-zinc-300" 
                  />
                </div>
              </div>
                  ...
                  {filteredCitiesList.map((city) => {
                    ...
                    return (
                      <button 
                        ...
                        className={cn(
                          "w-full flex items-center gap-3 p-2.5 rounded-2xl transition-all duration-300 group text-left border border-transparent",
                          isActive 
                            ? "bg-zinc-950 text-white shadow-xl border-zinc-800 translate-x-1" 
                            : "text-zinc-500 hover:bg-white hover:text-zinc-950 hover:shadow-md hover:border-zinc-100"
                        )}
                      >
                        <Avatar className={cn(
                          "w-8 h-8 border-2 border-white shadow-sm shrink-0 transition-transform group-hover:scale-110",
                          isActive && "ring-1 ring-orange-500 ring-offset-2 ring-offset-zinc-950"
                        )}>
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="font-black text-[10px] uppercase tracking-tight block truncate leading-none mb-1">{city}</span>
                          <span className={cn(
                            "text-[7px] font-bold uppercase tracking-widest block opacity-60",
                            isActive ? "text-orange-400" : "text-zinc-400"
                          )}>Maranhão</span>
                        </div>

                        <span className={cn(
                          "text-[9px] font-black tabular-nums px-2 py-1 rounded-xl min-w-[24px] text-center transition-colors",
                          isActive ? "text-orange-500 bg-zinc-900 border border-zinc-800" : "text-zinc-300 bg-zinc-100 group-hover:text-zinc-500 group-hover:bg-zinc-50"
                        )}>
                          {amCount}
                        </span>
                      </button>
              ...
              <div className="p-8 space-y-12 w-full pb-24">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start border-b border-zinc-50 pb-10">
                  ...
                  <div className="lg:col-span-9 space-y-6">
                    <div>
                      <h4 className="text-[10px] font-black text-orange-600 uppercase tracking-[0.4em] mb-2 flex items-center gap-3">
                        <span className="w-8 h-px bg-orange-200" /> Veredito Geral
                      </h4>
                    ...
                    <div className="grid grid-cols-3 gap-6 pt-4">
                      {[
                        ...
                        <div key={stat.label} className="space-y-2.5">
                          <div className="flex justify-between items-end">
                            <span className="text-[10px] font-black text-zinc-950 uppercase tracking-widest">{stat.label}</span>
                            <span className={cn("text-2xl font-black tracking-tighter tabular-nums", stat.text)}>
                              {stat.value.toFixed(1).replace('.', ',')}%
                            </span>
                          </div>
                          <div className="h-2 w-full bg-zinc-50 rounded-full overflow-hidden shadow-inner border border-zinc-100/50">
                            <motion.div 
                              initial={{ width: 0 }} 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <h4 className="text-[11px] font-black text-zinc-950 uppercase tracking-[0.4em]">Briefing de Inteligência</h4>
                    </div>

                    <div className="space-y-6">
                      <p className="text-xl font-black leading-tight tracking-tight text-zinc-900 uppercase">
                        A aprovação demonstra uma <span className="text-orange-600 underline decoration-orange-200 underline-offset-8">resiliência estratégica</span> nos principais polos regionais do estado.
                      </p>
                      ...
                      <div className="grid grid-cols-1 gap-6 pt-4">
                        <div className="space-y-2 group">
                          <div className="flex items-center gap-2 text-emerald-600">
                            <TrendingUp size={16} />
                            <span className="text-[9px] font-black uppercase tracking-[0.3em]">Oportunidade de Expansão</span>
                          </div>
                          <p className="text-xs text-zinc-500 font-bold leading-relaxed pr-10">
                            Crescimento orgânico de 4.2% detectado em áreas de influência direta, sugerindo eficiência máxima nos canais de comunicação institucional.
                          </p>
                        </div>
                        <div className="space-y-2 border-t border-zinc-50 pt-6 group">
                          <div className="flex items-center gap-2 text-rose-600">
                            <ShieldAlert size={16} />
                            <span className="text-[9px] font-black uppercase tracking-[0.3em]">Indicador de Risco Crítico</span>
                          </div>
                          <p className="text-xs text-zinc-500 font-bold leading-relaxed pr-10">
                            Sinal de alerta na Mesorregião Sul, onde a rejeição rompeu o teto histórico por gargalos específicos em infraestrutura e segurança rural.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-8 pt-6">
                  <div className="flex items-center justify-between border-b border-zinc-50 pb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-1.5 h-6 bg-orange-600 rounded-full" />
                      <h4 className="text-[11px] font-black text-zinc-950 uppercase tracking-[0.4em]">Radar Geoestatístico Detalhado</h4>
                    </div>
                    {filters.city[0] !== 'all' && (
                      <Badge className="bg-zinc-950 text-white font-black uppercase text-[10px] px-5 py-1.5 rounded-xl border-none shadow-lg">Foco: {filters.city[0]}</Badge>
                    )}
                  </div>
