...
export const InteractiveMap = ({ onRegionSelect, stats, activeRegion }: InteractiveMapProps) => {
  ...
  if (!apiKey || loadError) {
    return (
      <LuxuryCard title="Geolocalização" subtitle="Engine de Mapas" className="lg:col-span-2 lg:row-span-2 min-h-[300px]">
  ...
  return (
    <LuxuryCard title="Geolocalização" subtitle="Densidade Real" className="lg:col-span-2 lg:row-span-2 relative p-0 overflow-hidden min-h-[400px]">
      ...
      <div className="w-full h-full relative">
        {isLoaded ? (
          <GoogleMap mapContainerStyle={mapContainerStyle} center={center} zoom={5.5} options={mapOptions}>
        ...
        <AnimatePresence>
          {currentRegion && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="absolute left-4 right-4 bottom-4 lg:left-auto lg:right-6 lg:bottom-6 p-8 bg-white/95 backdrop-blur-xl border border-zinc-200 rounded-3xl shadow-2xl lg:min-w-[280px] z-30"
            >
